...
<!-- 聊天信息展示 -->
<div class="l-t-b">
<div class="man" v-for="(item,index) in xiaoxi" :key="index">{{item}}</div>
</div>
...
loginSuccess(data){
 ...
 this.act=1
 let b=data.username+"加入了群聊"
this.xiaoxi=this.xiaoxi.push(b)
},
loginError(data){
...
delUser(data){
 ...
 let a=data.username+"退出了群聊"
 this.xiaoxi=this.xiaoxi.push(a)
},
// 获取所有消息
receiveMessage(data){
  console.log("输入信息",data)
  let xiao=data
this.xiaoxi=this.xiaoxi.push(xiao)
  this.text=""
},
